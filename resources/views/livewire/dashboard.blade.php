<div class="min-h-screen bg-gradient-to-br from-kassako-cream to-forest-50/50">
    {{-- Header --}}
    <header class="bg-white/95 backdrop-blur-md border-b border-forest-100 sticky top-0 z-10">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between">
            <div class="flex items-center gap-3">
                <div class="w-10 h-10 bg-gradient-to-br from-kassako-forest to-forest-600 rounded-xl flex items-center justify-center shadow-sm">
                    <span class="text-white font-bold text-lg">K</span>
                </div>
                <div class="hidden sm:block">
                    <span class="font-serif text-xl text-kassako-text">Kassako</span>
                    <span class="text-kassako-muted text-sm ml-2">Dashboard</span>
                </div>
            </div>
            ...
            <div class="flex items-center gap-4">
                @if($lastUpdated)
                    <div class="hidden sm:flex items-center gap-2">
                        ...
                        <span class="text-xs text-kassako-muted">
                ...
                <button
                    ...
                    class="p-2.5 rounded-xl bg-forest-50 hover:bg-forest-100 transition-colors disabled:opacity-50 group"
                    title="Uppdatera data"
                >
                    <svg
                        class="w-5 h-5 text-kassako-forest group-hover:text-forest-700"
                ...
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="flex items-center gap-2 p-1.5 rounded-xl hover:bg-forest-50 transition-colors">
                            <div class="w-9 h-9 bg-gradient-to-br from-kassako-forest to-forest-600 rounded-full flex items-center justify-center">
                                <span class="text-white text-sm font-semibold">
                                    {{ substr(Auth::user()->name, 0, 1) }}
                                </span>
                            </div>
                            <svg class="w-4 h-4 text-kassako-muted hidden sm:block" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <x-slot name="content">
                        <div class="px-4 py-2 border-b border-gray-100">
                            <p class="text-sm font-medium text-kassako-text">{{ Auth::user()->name }}</p>
                            <p class="text-xs text-kassako-muted">{{ Auth::user()->currentTeam->name }}</p>
                        </div>
    ...
    <main class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
        @if(!$hasConnection)
            ...
            <div class="bg-white rounded-2xl border border-forest-100 shadow-card p-8 md:p-12 text-center max-w-2xl mx-auto">
                <div class="w-20 h-20 bg-gradient-to-br from-forest-50 to-kassako-gold/10 rounded-2xl flex items-center justify-center mx-auto mb-6">
                    <svg class="w-10 h-10 text-kassako-forest" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13.828 10.172a4 4 0 00-5.656 0l-4 4a4 4 0 105.656 5.656l1.102-1.101m-.758-4.899a4 4 0 005.656 0l4-4a4 4 0 00-5.656-5.656l-1.1 1.1"/>
                    </svg>
                </div>
                <h2 class="font-display text-3xl text-kassako-text mb-3">Koppla Fortnox</h2>
                <p class="text-kassako-muted mb-8 max-w-md mx-auto leading-relaxed">
        ...
        @elseif($isLoading)
            ...
            <div class="bg-white rounded-2xl border border-forest-100 shadow-card p-12 md:p-16 text-center">
                <div class="animate-spin w-10 h-10 border-3 border-kassako-forest border-t-transparent rounded-full mx-auto mb-4"></div>
                <p class="text-kassako-muted text-lg">Laddar din kassaprognos...</p>
            </div>
            ...
            <div class="space-y-6">
                ...
                <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-4">
                    ...
                    <div class="bg-white rounded-2xl p-5 border border-forest-100 shadow-card hover:shadow-card-hover transition-all duration-300 hover:-translate-y-1 group">
                        ...
                        <p class="font-display text-2xl md:text-3xl font-bold text-kassako-text">
                            ...
                            <span class="text-lg font-normal text-kassako-muted">kr</span>
                        </p>
                    ...
                    <div class="bg-white rounded-2xl p-5 border border-forest-100 shadow-card hover:shadow-card-hover transition-all duration-300 hover:-translate-y-1 group">
                        ...
                        <p class="text-kassako-muted text-xs mt-2">Nästa 30 dagar</p>
                    </div>
                ...
                <div class="grid lg:grid-cols-3 gap-4 md:gap-6">
                        ...
                        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
                            ...
                            <div class="flex items-center gap-2">
                                <button
                                    ...
                                    class="px-3 py-1.5 text-xs font-medium rounded-lg transition-colors {{ $chartPeriod === '12' ? 'bg-kassako-forest text-white' : 'bg-forest-50 text-kassako-forest hover:bg-forest-100' }}"
                                >
                                    12 mån
                                </button>
                                <button
                                    wire:click="setChartPeriod('6')"
                                    class="px-3 py-1.5 text-xs font-medium rounded-lg transition-colors {{ $chartPeriod === '6' ? 'bg-kassako-forest text-white' : 'bg-forest-50 text-kassako-forest hover:bg-forest-100' }}"
                                >
                                    6 mån
                                </button>
                                <button
                                    wire:click="setChartPeriod('3')"
                                    class="px-3 py-1.5 text-xs font-medium rounded-lg transition-colors {{ $chartPeriod === '3' ? 'bg-kassako-forest text-white' : 'bg-forest-50 text-kassako-forest hover:bg-forest-100' }}"
                                >
                                    3 mån
                                </button>
                            </div>
                    ...
                    <div class="bg-white rounded-2xl p-5 md:p-6 border border-forest-100 shadow-card">
                        ...
                        <div id="runway-radial-chart" class="h-48" wire:ignore></div>
                        <div class="mt-4 space-y-2">
                            ...
                            <div class="flex items-center justify-between text-sm">
                                ...
                                <span class="text-kassako-success font-medium">&gt; 60 dagar {{ $snapshot->runway_days > 60 ? '✓' : '' }}</span>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <div class="bg-gradient-to-br from-kassako-forest/5 to-kassako-gold/5 rounded-2xl p-5 md:p-6 border border-kassako-forest/20">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                ...
                                <p class="text-kassako-muted text-xs">Uppdaterad {{ $lastUpdated ?? 'nyss' }}</p>
                            </div>
                        </div>
                        <div class="space-y-3">
                            @forelse($snapshot->insights ?? [] as $insight)
                                @php
                                    $borderColor = match($insight['type'] ?? 'info') {
                                        'success' => 'border-kassako-success/20',
                                        'warning' => 'border-yellow-500/20',
                                        'danger' => 'border-red-500/20',
                                        default => 'border-kassako-forest/20',
                                    };
                                    $iconBg = match($insight['type'] ?? 'info') {
                                        'success' => 'bg-kassako-success/20',
                                        'warning' => 'bg-yellow-500/20',
                                        'danger' => 'bg-red-500/20',
                                        default => 'bg-kassako-forest/20',
                                    };
                                    $iconColor = match($insight['type'] ?? 'info') {
                                        'success' => 'text-kassako-success',
                                        'warning' => 'text-yellow-600',
                                        'danger' => 'text-red-500',
                                        default => 'text-kassako-forest',
                                    };
                                @endphp
                            ...
                            @empty
                                <div class="flex items-start gap-3 bg-white/60 rounded-xl p-3 border border-kassako-success/20">
                                    ...
                                    <p class="text-sm text-kassako-text"><strong>Allt ser bra ut!</strong> Inga varningar just nu.</p>
                                </div>
                            @endforelse
                        </div>
                    </div>
